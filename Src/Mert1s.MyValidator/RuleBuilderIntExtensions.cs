...
public static class RuleBuilderIntExtensions
{
    ...
    public static RuleBuilder<TInstance, int> GreaterThan<TInstance>(this RuleBuilder<TInstance, int> builder, int value) => builder.Must((x) => x > value, (x, _) => $"{x} is not greater than {value}");

    /// <summary>
    /// Garante que o valor seja maior ou igual a um número especificado.
    /// </summary>
    /// <param name="value">Valor mínimo permitido.</param>
    public static RuleBuilder<TInstance, int> GreaterThanOrEqual<TInstance>(this RuleBuilder<TInstance, int> builder, int value) => builder.Must(x => x >= value, (x, _) => $"{x} is not greater than or equal to {value}");
    ...
    public static RuleBuilder<TInstance, int> LessThanOrEqual<TInstance>(this RuleBuilder<TInstance, int> builder, int value) => builder.Must(x => x <= value, (x, _) => $"{x} is not less than or equal to {value}");
